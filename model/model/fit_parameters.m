function [ML_parameters, nLLVec, runlist_completed] = fit_parameters(model,data,exppriorityVec,runlist,runmax,fixparams,condVec)
...
for irun = 1:length(runlist)
    ...
    if strcmp(model(1:3),'all') % if fitting all conditions together
        ...
        [x,fval] = fmincon(fun,x0,[],[],[],[],lb,ub,[],optimset('Display','iter'));
